...
export const gameMono =
  "font-[family-name:var(--font-share-tech)] text-xs text-[var(--game-text-muted)]"
...
export function GameLogo({ className = "" }: { className?: string }) {
  return (
    ...
    >
      <span className="text-[var(--game-cyan)] [text-shadow:0_0_20px_rgba(65,245,240,0.55)]">
        Dé
      </span>
      <span className="text-[var(--game-magenta)] [text-shadow:0_0_18px_rgba(255,61,154,0.55)]">
        Fi
      </span>
    </div>
  )
...
export function GameStatPill({
  ...
  return (
    <div className="rounded-sm border border-[var(--game-cyan-dim)] bg-[rgba(4,2,12,0.75)] px-3 py-2">
      <p className="text-[9px] font-bold uppercase tracking-wider text-[var(--game-text-muted)]">
        {label}
      </p>
