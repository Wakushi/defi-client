...
import { parseReadyState } from "@/lib/db/duel-ready"
import { findDuelById, markDuelLiveIfUnset } from "@/lib/db/duels"
...
export async function POST(
  ...
  if (rs[0] !== 1 || rs[1] !== 1) {
    return NextResponse.json(
      { error: "Both players must be ready before marking duel live." },
      { status: 400 },
    )
  }

  await markDuelLiveIfUnset(duelId)
  const fresh = await findDuelById(duelId)
  const liveAt = fresh?.duel_live_at
  ...
  return NextResponse.json({
    ...
    duelLiveAt:
      liveAt instanceof Date
        ? liveAt.toISOString()
        : liveAt
          ? new Date(liveAt as string).toISOString()
          : null,
  })
}
